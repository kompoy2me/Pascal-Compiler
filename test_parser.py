...
class test_parser():
	...
	def printBinOpNode(self, node, field):
		...
		#self.res_str+=(" "*self.space + node.getValue().get_value()+"\n")
		#print(" "*self.space + node.getValue().get_value()+"\n")
		if node.getValue().get_type() == 'error':
			err_symb =  node.getValue().get_value()
			if err_symb == "":
				err_symb = "eof"
			self.res_str+=(str(node.getValue().get_pos())+ ' Error: Unexpected symbol '+ err_symb)
	...
	'''def printBinOpNode1(self, node):
		#print("%5s%5s%5s" % (node.getLeftNode().getValue(), node.getValue(), node.getRightNode().getValue()) + "\n")
		self.res_str+=(" "*self.space + node.getValue().get_value().center(self.space)+"\n")
		#self.res_str+=(" "*self.space + node.getValue().get_value()+"\n")
		#print(" "*self.space + node.getValue().get_value()+"\n")
		if node.getValue().get_type() == 'error':
			err_symb =  node.getValue().get_value()
			if err_symb == "":
				err_symb = "eof"
			self.res_str+=(node.getValue().get_pos(), 'Error: Unexpected symbol', err_symb)
			return
		if node.getLeftNode():
